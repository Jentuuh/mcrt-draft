...
namespace mcrt {
	...
	public:
		...
		void recursiveSplit(int currentLevel, int maxDepth, Scene& sceneObject, std::vector<float>& gpuOctree, std::stack<glm::vec3>& parentCoordStack, glm::ivec3& currentIndirectGridCoord, bool nodeIntersectsWithSceneGeometry, int* nodeCount, std::function<void(int)> progressCallback);
